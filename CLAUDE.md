...
## 20. iOS / Android native apps — React Native hybrid shell (2026-08-05 → 2026-08-18)
...
**Drawer currency + account after the 2026-08-18 main merge.** This work
originally removed the currency toggle from the TopHeader; the merge with
main's §19 header redesign (which carries its own compact `CurrencyMenu`
in the header) superseded that. Final state: the BROWSER keeps the §19
header exactly as main designed it (CurrencyMenu, bell, avatar). The
sidebar carries the shell/mobile complement — DRAWER-ONLY: a "Currency"
row with the `CurrencyToggle` pill above the Intelligence divider, no
System group (Settings reachable via the account row; Website dropped),
and an account row (initials avatar + name + email,
`data-testid="sidebar-account"`) wired via `onOpenAccount` → AppShell
closes the drawer and opens the **Command Center** account surface. The
desktop rail is untouched by all of this. Inside the native shell the
TopHeader isn't rendered at all, so the drawer is the one home for
currency + account there.
