# `archive/calibration_toolkit/` — the (formerly active) calibration oracle

> **ARCHIVED 2026-05-23 under F3.12.** Read `CLOSURE.md` next to this file
> for the formal close rationale and re-activation criteria. The README
> below is the original (point-in-time) document, retained verbatim
> because rewriting it would falsify the history. Path references inside
> this README point to `src/engine/api/_ro_coa.py` and other pre-F3.1c
> file names — these have since been refactored to the country-pack
> layout (`src/engine/country_packs/ro_romania/`). The directory itself
> was at `reference/` until F3.12 archived it here.
