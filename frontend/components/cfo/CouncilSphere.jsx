...
// Aligned to the real extraction-integrity council (see engine _ai_council.py):
// a presiding Chair plus the three auditor members whose ids match the
// backend event stream (reconciliation_auditor / completeness_auditor /
// classification_auditor).
export const SPHERE_MEMBERS = [
  { id: 'chair', name: 'The Chair', role: 'Consensus', color: '#8FE3D9' },
  { id: 'reconciliation_auditor', name: 'Reconciliation', role: 'P&L / BS ties', color: '#5CD3C5' },
  { id: 'completeness_auditor', name: 'Completeness', role: 'Coverage', color: '#2AA89B' },
  { id: 'classification_auditor', name: 'Classification', role: 'RAS mapping', color: '#8FE3D9' },
];
...
export const SPHERE_PACKET_COLORS = {
  doc: '#8FE3D9', pen: '#5CD3C5', fact: '#5CD3C5', chat: '#5CD3C5',
  flag: '#5CD3C5', ask: '#8A93A8', ok: '#5CD3C5', no: '#F87171',
};
...
const TEAL_STEPS = (() => {
  const pa = 0x8FE3D9; const pb = 0x2AA89B; const out = [];
  for (let i = 0; i <= 8; i++) {
    const t = i / 8;
...
export default class CouncilSphere extends React.Component {
  ...
  buildSphere() {
    ...
    for (let i = 0; i < NN; i++) {
      ...
      this.trees[pt.region].push({ a: pt.parent, b: i, birth: 0, fc: fu < 0.5 ? '#5CD3C5' : fu < 0.85 ? '#8FE3D9' : '#5CD3C5' });
    }
  ...
  draw(ts) {
    ...
    if (!halo) {
      ...
      halo.addColorStop(0, 'rgba(143, 227, 217, ' + (0.05 + (hq / 24) * 0.10) + ')');
      halo.addColorStop(1, 'rgba(143, 227, 217, 0)');
    ...
    const regionCols = this.MEMBERS.map((m) => (pw > 0.02 ? this.mixHex(m.color, '#8FE3D9', Math.min(1, pw)) : m.color));
    const pathPos = (o, t) => {
    ...
    for (const so of this.settledOrbs || []) {
      ...
      this.glowDot(ctx, pos[0], pos[1], 2.6 * pos[3], soCol, 6, '#8FE3D9');
    }
    ...
    for (const fn of this.fileNodes || []) {
      ...
      this.glowDot(ctx, pos[0], pos[1], fnR, '#8FE3D9', 6);
    ...
    for (let i = 0; i < this.pts.length; i++) {
      ...
      if (pt.sz > 2.6) {
        ctx.globalAlpha = Math.min(1, al);
        this.glowDot(ctx, pr[0], pr[1], s, dotCol, 8, con > 0.01 ? '#8FE3D9' : dotCol);
      } else {
    ...
    const pkColor = (pk) => (this.props.tintBySender
      ? (this.MEMBERS.find((m) => m.id === pk.fromId)?.color || '#8FE3D9')
      : this.PACKET_COLORS[pk.type] || '#8FE3D9');
    ...
    const pulse = this.corePulse || 0;
    {
      ...
      if (!flare) {
        ...
        flare.addColorStop(0.35, `rgba(143, 227, 217, ${0.45 + 0.15 * (fqC / 12)})`);
        flare.addColorStop(1, 'rgba(143, 227, 217, 0)');
